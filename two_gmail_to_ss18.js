...
function two_gmail_to_ss18() {
  ...
  try {
    ...
    //var attachment = threads[0].getMessages()[1].getAttachments()[0];
    console.log(arguments.callee.name + ": Emily's email attachment content type is %s, and the attachment file name is %s", attachment.getContentType(), attachment.getName())
    ...
    console.log(arguments.callee.name + ": The cantonese worship email message length is " + cant_message_length)
  ...
  if (success) {
    ...
    //console.log(arguments.callee.name + ": The Orig cantonese worship gmail thread, before splitting: %s ", cant_message_bef_split)
    cant_msg.reverse();
    console.log(arguments.callee.name + ": The cantonese worship gmail thread, with all messages body is: %s ", cant_msg)
    ...
    console.log(arguments.callee.name + ": Sorry, Can't find one of the email to populate the sermon info on the slide_src !!!");
  }

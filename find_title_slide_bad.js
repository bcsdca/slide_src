function find_title_slide_bad(slide_id, title_input) {
    ...
    elems.forEach(
      ((el, index) => {
        if (el.getPageElementType() == "SHAPE") {
          ...
          //Logger.log("slide" + i + ": txt = " + txt + ", Looking for title_str " + title_str + " at index = " + index);
          //if (txt.match(title_str)) {
          if (txt === title) {
            find_it = 1;
            Logger.log("Found it!!! slide=" + i + ": title_str = " + txt + " at index = " + index);
            //return slides[i];
          }
  ...
  if (find_it == 0) {
    Logger.log("Sorry, can't find title_str \"%s\" on all slides !!! " + txt);
  }

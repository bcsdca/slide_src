...
function two_gmail_to_ss12() {
  ...
  try {
    ...
    //cantonese worship email
    //var cant_threads = GmailApp.search('Cantonese Sunday Service(01/01/2023) Reminder', 0, 1);
  ...
  if (success) {
    ...
    for (var j = 0; ((j < cant_msg.length) && (find_it == 0)); j++) {
      //peggy has done this on "Call to Worship: 希伯來書 Hebrews 4:12-16" on 2/10/2023
      //so change it to case insensitive match
      if (cant_msg[j].match(/Call to worship/i)) {
    ...
    for (var j = 0; j < msg.length; j++) {
      ...
      if (msg[j].match("Invocation")) {
        ...
        array_a1 = msg[j].split('#');
        array_a2 = array_a1[1].split('. ');
    ...
    for (var i = 0; (i < sheetContents.length); i++) {
      if (sheetContents[i].toString().match(/TITLE_ANNOUNCEMENT/)) {
        ...
        for (var k = 0; k < anno_array.length; k++) {
          var find_it = 0;
          var announcment_section_reached = 0;
          for (var j = 0; ((j < msg_pdf.length) && (find_it == 0)); j++) {
            if (msg_pdf[j].match("cprograms")) {
              announcment_section_reached = 1;
            }
            else if ((msg_pdf[j].match(anno_array[k])) && (announcment_section_reached == 1)) {
              ...
              //checking if there is another line after for this announcement
              if (!msg_pdf[j+1].match(/\d\./)) {
                console.log("Finding this next line is actually just a continuation of the last announcement: %s", msg_pdf[j+1])
                //anno_message_array = anno_message_array.concat(msg_pdf[j+1])
                anno_message_array = anno_message_array + " " + msg_pdf[j+1]
              }

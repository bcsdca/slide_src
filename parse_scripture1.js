function parse_scripture1(p) {
  ...
  for (var j = 0; j < a_temp1.length; j++) {
    ...
    a0 = a_temp1[j].split(":");
    //normally and there should be only one ":"s in the scripture reading string
    ...
    if ((a0.length == 2) || (a0.length == 1)) {
      ...
      if (a2[0].match(/腓利門書|約翰二書|約翰三書|猶大書|俄巴底亞書/)) {
        var book = a2[0];
        var chapter = 1;
        var verse = a2[1]
      } else if (a2.length == 4) {
        ...
        Logger.log("This scripture reading passage format1 is not supported %s", a_temp1[j])
        break;
      }
      ...
      Logger.log("Scripture reading passage return1 = %s", return1)
      array_return.push(return1)

    } else {
      //more than one ":" in the scripture reading
      Logger.log("This scripture passage with more than 1 \":\" is not supported %s", a_temp1[j])
      break;
    }
  ...
  Logger.log("Scripture reading passage array_return from parse_scripture function = %s", array_return)
  return array_return;
}

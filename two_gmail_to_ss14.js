...
//change Psalm 151 tp psalm 1xx because Wellington was confused on Psalm 151 on 3/18/2023
//replace (cont.) with (續) for extending announcment page (3/20/2023)
//replace("cec sd.org", "cec-sd.org") (3/20/2023)

function two_gmail_to_ss14() {
  ...
  try {
    ...
    //cantonese worship email
    //cant_query = 'is:inbox subject:"Cantonese Sunday Service(12/18/2022) Reminder"';
    //var cant_threads = GmailApp.search('is:inbox subject:"Cantonese Sunday Service(03/05/2023) Reminder"', 0, 1);
  ...
  if (success) {
    ...
    for (var j = 0; ((j < cant_msg.length) && (find_it == 0)); j++) {
      ...
      if ((cant_msg[j].match(/Call to worship/i)) && (!cant_msg[j].match("1xx"))) {

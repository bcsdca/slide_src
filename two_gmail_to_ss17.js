...
//Taking care if there are multiple messages for Emily's email and used only the 1st attachment from the initial(1st message)。(4/33/2023)

function two_gmail_to_ss17() {
  ...
  try {
    ...
    console.log("The Emily's worship email message length is " + message_length)
    ...
    console.log("Emily's email attachment content type is %s, and the attachment file name is %s", attachment.getContentType(), attachment.getName())
    
   
    //cantonese worship email
    //cant_query = 'is:inbox subject:"Cantonese Sunday Service(04/02/2023) Reminder"';
    //var cant_threads = GmailApp.search('is:inbox subject:"Cantonese Sunday Service(" + coming_sunday + ") Reminder"', 0, 1);
  ...
  if (success) {
    ...
    for (var j = 0; ((j < cant_msg.length) && (find_it == 0)); j++) {
      ...
      if ((cant_msg[j].match(/Call\s*to\s*worship/i)) && (!cant_msg[j].match("1xx"))) {
        ...
        console.log("The passage to read are %s, and the number of passage to read is %d ", array_invo_passage, array_invo_passage.length)
        ...
        for (var k = 0; k < array_invo_passage.length; k++) {
          ...
          console.log("Invocation bible gateway search passage is " + array_invo_passage[k])

          invo_passage_array = passage_capture2(array_invo_passage[k]);
          console.log("Invocation passage array returning from the reading subroutine is " + invo_passage_array + ", for the passage " + array_invo_passage[k])
    ...
    for (var j = 0; j < msg.length; j++) {
      ...
      if (msg[j].match("Invocation")) {
        ...
        var array_passage = parse_scripture1(msg[j]);
        ...
        if ((msg[j + 1].match(/\w/)) && !(msg[j + 1].match("Speaker"))) {
          ...
          console.log("Sermon title 2nd line is " + msg[j + 1])
        } else {
      ...
      else if (msg[j].match(/Emily Ip/i)) {
        console.log("Yea, Done with parsing Emily's worship info C-xxxxxx email !!!")
        break;
      }
    ...
    Logger.log("Yea, Done with populating the sermon info on the slide_src !!!");
  } else {
    SpreadsheetApp.getActive().toast("Sorry, Can't find one of the email to populate the sermon info on the slide_src !!!", "👎");
    Logger.log("Sorry, Can't find one of the email to populate the sermon info on the slide_src !!!");
  }

...
function two_gmail_to_ss13() {
  ...
  try {
    ...
    //cantonese worship email
    var cant_threads = GmailApp.search('subject:"Cantonese Sunday Service(03/05/2023) Reminder"', 0, 1);
  ...
  if (success) {
    ...
    for (var j = 0; ((j < cant_msg.length) && (find_it == 0)); j++) {
      //peggy has done this on "Call to Worship: 希伯來書 Hebrews 4:12-16" on 2/10/2023
      //so change it to case insensitive match
      //psalm 151 is used by example invocation format will be eliminated here
      if ((cant_msg[j].match(/Call to worship/i)) && (!cant_msg[j].match("151"))) {
